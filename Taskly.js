...
function showPageLoading() {
    ...
    loadingEl.className = 'page-loading active';
    loadingEl.innerHTML = `
        <div class="loader"></div>
        <p style="color: var(--text-primary); font-weight: 600; margin-top: 10px;">Loading...</p>
    `;
...
async function showPage(id) {
    ...
    // Show loading for heavy pages
    if (['profilePage', 'trashPage', 'dashboardPage'].includes(id)) {
        showPageLoading();
    }
    ...
    if (id === 'profilePage') {
        ...
        await loadTrashPage();
        hidePageLoading();
    } else if (id === 'dashboardPage') {
        await loadTasks();
        hidePageLoading();
    } else {
        // Hide loading for other pages
...
async function changePassword() {
    ...
    const changeBtn = document.querySelector('#changePasswordPage .btn-primary');
...
function simulateGoogleSignInFlow(isRegister = false) {
    ...
    if (isLoginPage) {
        googleContent.innerHTML = `
            ...
            <div style="display: flex; gap: 10px; justify-content: center;">
                <button id="googleCancel" style="padding: 12px 24px; background: #f0f0f0; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s ease;">
                    Cancel
                </button>
                <button id="googleContinue" style="padding: 12px 24px; background: #4285F4; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s ease;">
                    Continue
                </button>
            </div>
        `;
    ...
    document.getElementById('googleCancel').addEventListener('click', () => {
        googleModal.style.animation = 'fadeIn 0.3s ease reverse';
        setTimeout(() => {
...
function createGoogleAccountWithPassword(googleEmail) {
    ...
    passwordContent.innerHTML = `
        ...
        <div style="display: flex; gap: 10px; justify-content: center;">
            <button id="googleCreateCancel" style="padding: 12px 24px; background: #f0f0f0; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s ease;">
                Cancel
            </button>
            <button id="googleCreateAccount" style="padding: 12px 24px; background: #4285F4; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s ease;">
                Create Account
            </button>
        </div>
    `;
    ...
    document.getElementById('googleCreateCancel').addEventListener('click', () => {
        passwordModal.style.animation = 'fadeIn 0.3s ease reverse';
        setTimeout(() => {
...
async function saveEditProfile() {
    ...
    const editBioFull = document.getElementById('editBioFull');
    const editProfilePreviewFull = document.getElementById('editProfilePreviewFull');
    const saveBtn = document.getElementById('saveEditFull');
...
function openEditEmailModal() {
    ...
    content.innerHTML = `
        ...
        <div style="display: flex; gap: 10px; justify-content: center;">
            <button id="cancelEmailChange" style="padding: 12px 24px; background: #f0f0f0; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s ease;">
                Cancel
            </button>
            <button id="saveEmailChange" style="padding: 12px 24px; background: #4285F4; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; transition: all 0.3s ease;">
                Update Email
            </button>
        </div>
    `;
...
function initializeEventListeners() {
    ...
    document.getElementById('googleSign')?.addEventListener('click', () => {
        ...
        setTimeout(() => {
            ...
            simulateGoogleSignInFlow(false);
        }, 300);
    });
